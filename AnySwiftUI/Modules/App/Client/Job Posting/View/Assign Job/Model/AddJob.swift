...
//  AnySwiftUI
//
//  Created by Arbaz  on 07/05/26.
//

import Foundation

struct Api_AddJob : Codable {
    let result : Res_AddJob?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(Res_AddJob.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_AddJob : Codable {
    let id : String?
    let user_id : String?
