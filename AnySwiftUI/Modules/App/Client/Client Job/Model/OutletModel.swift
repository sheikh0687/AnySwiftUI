...
struct Api_ClientOutlet : Codable {
    let result : [Res_ClientOutlet]?
    let complete_shift_count : Int?
    let pending_shift_count : Int?
    let accepted_shift_count : Int?
    let status : String?
    let message : String?

    enum CodingKeys: String, CodingKey {
        case result = "result"
        case complete_shift_count = "complete_shift_count"
        case pending_shift_count = "pending_shift_count"
        case accepted_shift_count = "accepted_shift_count"
        case status = "status"
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent([Res_ClientOutlet].self, forKey: .result)
        complete_shift_count = try values.decodeIfPresent(Int.self, forKey: .complete_shift_count)
        pending_shift_count = try values.decodeIfPresent(Int.self, forKey: .pending_shift_count)
        accepted_shift_count = try values.decodeIfPresent(Int.self, forKey: .accepted_shift_count)
        status = try values.decodeIfPresent(String.self, forKey: .status)
        message = try values.decodeIfPresent(String.self, forKey: .message)
    }
}

struct Res_ClientOutlet : Codable {
    let id : String?
...
extension Res_ClientOutlet {
    static func makeDefault(id: String, businessName: String, businessLogo: String) -> Res_ClientOutlet? {
        let json: [String: Any] = [
            "id": id,
        ...
        guard let data = try? JSONSerialization.data(withJSONObject: json),
              let outlet = try? JSONDecoder().decode(Res_ClientOutlet.self, from: data) else {
            return nil
        }
        return outlet
    }
