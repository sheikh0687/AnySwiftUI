...
struct Api_ChangeJobRequestStatus : Codable {
    let result : Res_ChangeJobRequestStatus?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(Res_ChangeJobRequestStatus.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_ChangeJobRequestStatus : Codable {
    let id : String?
    let card_id : String?
