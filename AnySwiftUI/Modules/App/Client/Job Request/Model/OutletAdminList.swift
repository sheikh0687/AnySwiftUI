...
struct Api_OutletAdminList : Codable {
    let result : [Res_OutletAdminList]?
    let status : String?
    let message : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent([Res_OutletAdminList].self, forKey: .result)
        status = try values.decodeIfPresent(String.self, forKey: .status)
        message = try values.decodeIfPresent(String.self, forKey: .message)
    }
}

struct Res_OutletAdminList : Codable {
    let id : String?
