...
struct Api_DeleteWorkerShift : Codable {
    let result : Res_DeleteWorkerShift?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(Res_DeleteWorkerShift.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_DeleteWorkerShift : Codable {
    let id : String?
    let card_id : String?
