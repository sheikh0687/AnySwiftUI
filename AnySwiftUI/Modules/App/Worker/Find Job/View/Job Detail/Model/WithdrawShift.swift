...
//  AnySwiftUI
//
//  Created by Arbaz  on 10/04/26.
//

import Foundation

struct Api_WithdrawShift : Codable {
    let result : Res_WithdrawShift?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent(Res_WithdrawShift.self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_WithdrawShift : Codable {
    let id : String?
    let card_id : String?
