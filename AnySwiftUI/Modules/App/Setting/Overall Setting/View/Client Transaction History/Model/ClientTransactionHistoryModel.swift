//
//  ClientTransactionHistoryModel.swift
//  AnySwiftUI
//
...
struct Api_ClientTransactionHistory : Codable {
    let total_job : Int?
    let total_earning : Int?
    let result : [Res_ClientTransactionHistory]?
    let message : String?
    let status : String?

    enum CodingKeys: String, CodingKey {

        case total_job = "total_job"
    ...
    init(from decoder: Decoder) throws {
        ...
        total_job = try values.decodeIfPresent(Int.self, forKey: .total_job)
        total_earning = try values.decodeIfPresent(Int.self, forKey: .total_earning)
        result = try values.decodeIfPresent([Res_ClientTransactionHistory].self, forKey: .result)
...
struct Res_ClientTransactionHistory : Codable {
    let id : String?
    let card_id : String?
